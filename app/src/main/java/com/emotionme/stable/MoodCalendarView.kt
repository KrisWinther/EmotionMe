...
class MoodCalendarView @JvmOverloads constructor(
    ...
    private val colorToday = "#476fff".toColorInt()   // синий контур -> сегодня
    ...
    private fun drawLegend(canvas: Canvas, startX: Float, y: Double) {
        val items = listOf(
            colorHappy to "Счастье",
            colorJoy to "Радость",
            colorPeace to "Умиротворение",
            colorNeutral to "Нейтрально",
            colorTired to "Усталость",
            colorSad to "Грусть",
        ...
        val dotR = 6f * dp
        val gapX = 10f * dp
        val gapY = 24f * dp // Отступ между рядами

        // Делим элементы на две группы
        val mid = (items.size + 1) / 2
        val rows = listOf(items.take(mid), items.drop(mid))

        rows.forEachIndexed { index, rowItems ->
            // Считаем ширину текущей строки для центрирования
            val rowWidth = rowItems.sumOf { (_, label) ->
                (dotR * 2 + 4 * dp + legendPaint.measureText(label) + gapX).toDouble()
            }.toFloat() - gapX

            var currentX = (width - rowWidth) / 2f
            val currentY: Float = (y + (index * gapY)).toFloat()

            rowItems.forEach { (color, label) ->
                cellPaint.color = color
                canvas.drawCircle(currentX + dotR, currentY, dotR, cellPaint)
                currentX += dotR * 2 + 4 * dp
                canvas.drawText(label, currentX, (currentY + dotR * 0.4f).toFloat(), legendPaint)
                currentX += legendPaint.measureText(label) + gapX
            }
        }
    }
    ...
    private fun moodColor(mood: String?): Int = when {
        ...
        mood.contains("\uD83E\uDD70") || mood.contains("Счастье", ignoreCase = true) -> colorHappy
        mood.contains("😊") || mood.contains("Радость", ignoreCase = true) -> colorJoy
        mood.contains("\uD83D\uDE07") || mood.contains("Умиротворение", ignoreCase = true) -> colorPeace
        mood.contains("😐") || mood.contains("Нейтрально", ignoreCase = true) -> colorNeutral
        mood.contains("\uD83D\uDE34") || mood.contains("Усталость", ignoreCase = true) -> colorTired
